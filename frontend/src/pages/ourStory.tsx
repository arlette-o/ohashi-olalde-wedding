...
// `imageList.ts` is generated by src/scripts/imageList.py, which records paths
// the way they sit on disk ("public/mosaicpics/x.jpg"). Vite serves everything
// under public/ from the site root, so the leading "public/" has to come off or
// every request 404s.
const toPublicUrl = (path: string) => `/${path.replace(/^public\//, "")}`;

// Captions live in the translation files under `OurStory.mosaic`, keyed by
// filename so adding or reordering photos can't shuffle them onto the wrong
// picture. Photos without a caption simply render without a bar.
const fileName = (path: string) => path.split("/").pop() ?? path;

const itemData = imageList.map((item) => ({
  img: toPublicUrl(item.img),
  key: fileName(item.img),
  rows: item.rows,
  cols: item.cols,
}));
...
export default function OurStory() {
  ...
  const captions: Record<string, string> = lang("OurStory.mosaic") ?? {};
